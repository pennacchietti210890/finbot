...
async def test_create_graph():
    ...
    with patch("app.finbot.graphs.StateGraph") as mock_state_graph, patch(
        "app.finbot.graphs.MemorySaver"
    ) as mock_memory_saver, patch(
        "app.finbot.graphs.make_supervisor_node"
    ) as mock_make_supervisor:
        # Configure mocks
        ...
        assert (
            mock_graph_builder.add_node.call_count >= 9
        )  # 1 supervisor + 8 worker nodes
        assert (
            mock_graph_builder.add_edge.call_count >= 1
        )  # At least START -> supervisor
...
async def test_graph_stock_price_query(mock_graph_builder, mock_llm):
    ...
    with patch("app.finbot.graphs.StateGraph", return_value=mock_builder), patch(
        "app.finbot.graphs.MemorySaver", return_value=MagicMock()
    ), patch("app.finbot.graphs.make_supervisor_node", return_value=MagicMock()), patch(
        "app.finbot.nodes.stock_price_node", return_value=AsyncMock()
    ), patch(
        "app.finbot.nodes.stock_price_chart_node", return_value=MagicMock()
    ):
        # Create the graph
        graph = create_graph(mock_llm)
...
async def test_graph_invalid_query(mock_graph_builder, mock_llm):
    ...
    mock_ainvoke.return_value = {
        "messages": [
            ...
            {
                "role": "assistant",
                "content": "Your query cannot be processed with the current tools of this financial bot.",
                "name": "system",
            },
        ],
        "stock_ticker": "",
    ...
    with patch("app.finbot.graphs.StateGraph", return_value=mock_builder), patch(
        ...
        assert result["messages"][1]["content"].startswith(
            "Your query cannot be processed"
        )
...
async def test_full_graph_workflow_integration():
    ...
    with patch(
        "app.finbot.nodes.LLMService", return_value=MagicMock(client=mock_llm)
    ), patch(
        "app.finbot.nodes.asyncio.create_subprocess_exec", return_value=mock_proc
    ), patch(
        "app.finbot.nodes.agents_llm", mock_llm
    ):
        ...
        supervisor_node = make_supervisor_node(
            mock_llm, ["stock_price_data", "stock_price_chart"]
        )
