...
@pytest.mark.asyncio
async def test_stock_price_node_fetches_data(
    mock_llm, mock_state, mock_proc, mock_tools
):
    ...
    with patch(
        ...
        "app.finbot.nodes.load_tools_from_mcp_server", return_value=[mock_tools]
    ), patch(
        "app.finbot.nodes.ReActAgent"
    ) as mock_react_agent:
