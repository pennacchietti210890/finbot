...
def mock_yf_ticker():
    ...
    mock_ticker.balance_sheet.to_dict.return_value = {
        pd.Timestamp("2023-01-01"): {"assets": 1000}
    }

    mock_ticker.financials = MagicMock()
    mock_ticker.financials.to_dict.return_value = {
        pd.Timestamp("2023-01-01"): {"revenue": 500}
    }

    mock_ticker.cashflow = MagicMock()
    mock_ticker.cashflow.to_dict.return_value = {
        pd.Timestamp("2023-01-01"): {"operating_cash": 200}
    }
...
def test_get_historical_prices_success(mock_yf_ticker):
    ...
    with patch(
        "app.finbot.mcp_finbot.mcp_tools.yf.Ticker", return_value=mock_yf_ticker
    ):
...
def test_get_historical_prices_exception():
    ...
    with patch(
        "app.finbot.mcp_finbot.mcp_tools.yf.Ticker", side_effect=Exception("Test error")
    ):
        result = get_historical_prices("AAPL", 30)
        result_json = json.loads(result)
...
def test_get_macroeconomic_series_success():
    ...
    with patch(
        "app.finbot.mcp_finbot.mcp_tools.pdr.get_data_fred",
        return_value=mock_macro_data,
    ):
...
def test_search_news_success():
    ...
    with patch(
        "app.finbot.mcp_finbot.mcp_tools.requests.post", return_value=mock_response
    ):
...
def test_search_news_exception():
    ...
    with patch(
        "app.finbot.mcp_finbot.mcp_tools.requests.post",
